...
def plot_stats(stats, blstates, rstats, smoothing_window=30, noshow=False):
    ...
    weights_smoothed = pd.Series(stats.trust_radius * 100).rolling(smoothing_window, min_periods = smoothing_window).mean()
    ...
    weights_smoothed = pd.Series(stats.trust_radius * 1000).rolling(smoothing_window, min_periods = smoothing_window).mean()
    plt.plot(weights_smoothed, color='g', linestyle='-', label="weights")
    plt.legend()
    plt.xlabel("Episode")
