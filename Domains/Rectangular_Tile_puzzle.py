...
class RectangularTile(AbstractState):
    m = 2                             #TODO: make code generic to m
    def __init__(self, board, g):
        self.board = board
        assert len(board) % RectangularTile.m == 0
        self.dim = (len(board)) // RectangularTile.m
        super(RectangularTile,self).__init__(g)
    ...
    def get_successors(self):
        ...
        for op in range(4):
            ...
            #print(n, blank, len(c_board), op)
            if n is not None:
                c_board[blank], c_board[n] = c_board[n], c_board[blank]
                child = RectangularTile(c_board, self.g + 1)
    ...
    def is_solution(self):
        ...
        return self == RectangularTile.get_goal_dummy(self.dim)
    ...
    def parse_state(string):
        ...
        for n in string:
            board.append(int(n))
        return RectangularTile(board,0)

    @staticmethod
    def get_goal_dummy(size):
        tiles = RectangularTile.m * size
